//! Operator-facing CLI for the Drive OAuth flow.
//!
//! Phase 123 Task 3. Three subcommands:
//!
//! - `aivyx-drive auth init` — runs the auth-code flow. Loads
...
//!
//! - `aivyx-drive auth status` — loads tokens (if any) and
//!   prints a human-readable health report: granted scopes,
//!   expiry, refresh-available?, time-since-init.
//!
//! - `aivyx-drive auth revoke` — calls Google's revoke
...
//!   config from `~/.aivyx-pa/tool-processes/drive/config.toml`.
