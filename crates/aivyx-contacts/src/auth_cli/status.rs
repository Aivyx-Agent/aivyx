...
//! - Granted scope (so the operator can confirm `contacts`
...
fn format_status_report(tokens: &TokenSet, path: &Path, now_unix_secs: i64) -> String {
    ...
    format!(
        "Aivyx PA Contacts — token status\n\
         --------------------------\n\
         Token file:        {path}\n\
         Granted scope:     {scope}\n\
...
fn format_no_tokens_message(path: &Path) -> String {
    format!(
        "Aivyx PA Contacts — token status\n\
         ...
         Run `aivyx-contacts auth init` to complete the OAuth flow.\n",
        path = path.display(),
    )
}
...
mod tests {
    ...
    fn sample_tokens() -> TokenSet {
        TokenSet {
            ...
            granted_scope: "https://www.googleapis.com/auth/contacts https://www.googleapis.com/auth/contacts.readonly".to_string(),
    ...
    fn format_status_report_renders_fresh_token() {
        ...
        assert!(out.contains("Aivyx PA Contacts — token status"));
        assert!(out.contains("Token file:        /tmp/t.json"));
        assert!(out.contains("contacts"));
        assert!(out.contains("contacts.readonly"));
    ...
    fn format_no_tokens_message_points_at_auth_init() {
        ...
        assert!(out.contains("aivyx-contacts auth init"));
    }
