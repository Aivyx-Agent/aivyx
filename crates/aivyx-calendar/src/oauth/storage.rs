...
//! process file at `~/.aivyx/tool-processes/calendar/tokens.json`
//! with 0600 perms.
//!
//! ## Why a file, not the redb store
//!
//! Original Q3 pick was the encrypted redb store under
//! `KeyDomain::Calendar`. Post-Q-block sign-off, P10's
//! "email-is-third-party" rule surfaced and Calendar must ship as
...
/// Default path for the Calendar tool process's token file.
/// Resolves to `$HOME/.aivyx/tool-processes/calendar/tokens.json`.
...
mod tests {
    ...
    fn sample_tokens() -> TokenSet {
        TokenSet {
            ...
            granted_scope: "https://www.googleapis.com/auth/calendar.readonly".to_string(),
