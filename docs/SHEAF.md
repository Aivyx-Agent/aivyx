...
| **SH.1** | **`data.csv` + the sandbox-reuse spine** | First reader *and* the path the others reuse: stand up `aivyx-dataread`, wire the `FsReadTool`-style `fs.read` scope + `lexical_resolve` fence, parse CSV (already-in-tree `csv` crate) → `{headers, rows, row_count, truncated}`. Proves the reuse pattern once. |
| **SH.2** | **`data.xlsx`** | Add `calamine` (xlsx, MIT — cleared by `cargo deny`); a named sheet (or first sheet) → `{sheet, headers, rows, truncated}`. |
