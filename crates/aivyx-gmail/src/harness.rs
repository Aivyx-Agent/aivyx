//! Multi-tool IPC harness — Phase 123 SDK-validation finding.
//!
//! Phase 123 Task 4. The existing
//! [`aivyx_tool::run_tool_as_subprocess`] harness wraps ONE
//! [`aivyx_core::Tool`] and was sufficient for Phase 49's
//! single-fixture-tool foundation phase. The reality of
//! Chapter F integrations is that one tool process naturally
//! serves multiple tools (Gmail has four; Calendar will have
//! several; Drive will have many). This harness mirrors
//! `run_tool_as_subprocess`'s structure but dispatches
//! `InvokeTool { tool_name, ... }` against a name-keyed
//! `HashMap<String, Arc<dyn Tool>>`.
//!
//! **Phase 123's load-bearing SDK-validation finding.** This
//! harness is the first reusable proof that a multi-tool
//! process is the natural shape for third-party integrations.
//! The Task 8 exit doc surfaces the lift-to-substrate
//! recommendation: a future phase should generalize this into
//! `aivyx_tool::run_tools_as_subprocess(Vec<Arc<dyn Tool>>, …)`
//! and have Gmail / Calendar / Drive consume the shared
//! substrate.
...
/// No-op `ChannelContext` for tool-side dispatch. Phase 123's
/// Gmail tools don't stream progress events — each is a single
/// HTTP call. A streaming Chapter F integration (e.g. a
/// future `drive.upload` with chunked progress) would replace
