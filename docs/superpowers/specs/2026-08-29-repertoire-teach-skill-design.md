...
**Corrected during planning — a real "create new item" form precedent does exist.** This section originally claimed none did; re-checked while writing the implementation plan and found `SchedulesPanel`'s own "Create schedule" form (main.rs, search `let mut cron = use_signal(String::new);`) is exactly this shape. It keeps its text fields as **local** `use_signal`s inside the component and uses its shared `SchedulesUi` context signal only for `notice` — not the shared-struct-holds-everything shape this doc originally proposed. The implementation plan (`docs/superpowers/plans/2026-08-29-repertoire-teach-skill.md`) follows the real precedent: `SkillsUi` holds only `notice`; the form's `name`/`trigger`/`procedure`/open-toggle are local signals in `SkillsPanel`.
