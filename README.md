...
- **The Studio — a local-first web GUI.** Fifteen offline, Stitch-styled screens
  served on `:7843`: Create (guided agent onboarding), Command Center, Missions,
  Mission Control (a live team mission's LEAD/specialist graph, drill-in, and
  abort/pause/resume controls), Chat, Memory (+ co-occurrence graph), Wiki
  (synthesized knowledge pages), Graph (typed knowledge graph), Skills (the
  skill library + effectiveness), Settings, Agents, Teams, Documents (browse +
