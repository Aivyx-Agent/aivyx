//! `aivyx-contacts auth init` — run the OAuth authorization-code
//! flow.
//!
...
pub async fn await_callback(
    ...
    let fut = async {
        ...
        write_browser_response(
            &mut write_half,
            "Aivyx PA Contacts — authorization received",
...
pub async fn run_auth_init(
    ...
    println!(
        "\n\
         Aivyx PA Contacts — OAuth consent required\n\
         ------------------------------------\n\
    ...
    println!(
        "Tokens saved to {token_path:?}.\n\
         Granted scope: {}\n\
         Run `aivyx-contacts auth status` to inspect.",
...
mod tests {
    ...
    fn sample_config() -> OAuthConfig {
        ...
        // contacts-specific default set so consent-URL
        ...
        .with_scopes(crate::DEFAULT_CONTACTS_SCOPES.iter().copied())
    }
