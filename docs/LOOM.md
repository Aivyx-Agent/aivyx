...
> source-labeled breadcrumb (`aivyx recall-graph: …`) shows when the graph
...
| **LM.5** ✅ | **Eval harness + finalize** | DONE. A `recall@k` eval harness in `memory_recall.rs` (a `ConstProvider` for deterministic semantics + a `recall_fraction` helper over a `(query, expected)` fixture set) proves graph-augmented fusion recalls **1.0** vs semantic-only's **<0.5** — the lexical-only (rare term) and graph-only (associated topic) targets are exactly what semantic-only drops. Source-labeled breadcrumb (`aivyx recall-graph: injected N via graph-walk`). Full workspace suite + clippy + `cargo deny` (licenses + advisories) green; zero new deps; banner flipped; recorded. |
