//! CLI argument parsing for `aivyx-gmail auth ...`.
//!
...
pub enum AuthMode {
    /// `aivyx-gmail auth init` — run the OAuth flow.
    Init,
    /// `aivyx-gmail auth status` — print token health.
    Status,
    /// `aivyx-gmail auth revoke` — revoke + delete tokens.
    Revoke,
}
...
fn parse_auth(args: &[String]) -> Result<BinaryMode, String> {
    ...
    match sub {
        ...
        "" => Err(
            "`aivyx-gmail auth` requires a subcommand; \
...
pub fn help_text() -> &'static str {
    "aivyx-gmail — Gmail third-party tool process for Aivyx PA

USAGE:
    aivyx-gmail [SUBCOMMAND]

SUBCOMMANDS:
    auth init      Run the Google OAuth consent flow and save
                   tokens to `~/.aivyx-pa/tool-processes/gmail/tokens.json`.
                   Requires `~/.aivyx-pa/tool-processes/gmail/config.toml`
    ...
    auth revoke    Call Google's revoke endpoint and delete the
                   ...
                   before any Gmail tool will work.
...
expects to be spawned by the Aivyx PA daemon via a
