...
async fn two_concurrent_connections() {
    ...
    let daemon_shutdown = shutdown.clone();
    let daemon_handle = tokio::spawn(async move {
        run_daemon(&daemon_socket, daemon_agent, daemon_factory, daemon_shutdown, None, None, None, None)
            .await
            .expect("daemon must complete successfully");
    });
...
async fn telegram_frontend_type_gets_telegram_channel() {
    ...
    let daemon_shutdown = shutdown.clone();
    let daemon_handle = tokio::spawn(async move {
        run_daemon(&daemon_socket, daemon_agent, factory, daemon_shutdown, None, None, None, None)
            .await
            .expect("daemon must complete successfully");
    });
