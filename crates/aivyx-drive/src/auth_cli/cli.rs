//! CLI argument parsing for `aivyx-drive auth ...`.
//!
...
pub enum AuthMode {
    /// `aivyx-drive auth init` — run the OAuth flow.
    Init,
    /// `aivyx-drive auth status` — print token health.
    Status,
    /// `aivyx-drive auth revoke` — revoke + delete tokens.
    Revoke,
}
...
fn parse_auth(args: &[String]) -> Result<BinaryMode, String> {
    ...
    match sub {
        ...
        "" => Err(
            "`aivyx-drive auth` requires a subcommand; \
...
pub fn help_text() -> &'static str {
    "aivyx-drive — Drive third-party tool process for Aivyx PA

USAGE:
    aivyx-drive [SUBCOMMAND]

SUBCOMMANDS:
    auth init      Run the Google OAuth consent flow and save
                   tokens to `~/.aivyx-pa/tool-processes/drive/tokens.json`.
                   Requires `~/.aivyx-pa/tool-processes/drive/config.toml`
    ...
    auth revoke    Call Google's revoke endpoint and delete the
                   ...
                   before any Drive tool will work.
