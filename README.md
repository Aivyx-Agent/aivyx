...
| Release pipeline | **Active** — on each version tag, cargo-dist builds the CLI (Linux x86_64/aarch64 musl + macOS x86_64/aarch64) and a separate workflow builds the **desktop app** (`.deb` + macOS `.app`); both attach to the GitHub Release. Latest is **`v0.8.1`** (the Vitrine harvest: conversation-history replay for chat sessions, skill trigger injection so skills are actually used, a deterministic identifier backstop in the completion judge, MCP tools in team-specialist hands, and a dozen honesty fixes from one day of live operator walkthrough) via the [shell installer](docs/INSTALL.md#shell-installer-recommended), the [desktop app](docs/INSTALL.md#desktop-app), or the [WSL distro](docs/INSTALL.md#windows-wsl2-or-docker) |
...
- **The Studio — a local-first web GUI.** Thirteen offline, Stitch-styled screens
  served on `:7843`: Create (guided agent onboarding), Command Center, Missions,
  Chat, Memory (+ co-occurrence graph), Wiki (synthesized knowledge pages), Graph
  (typed knowledge graph), Skills (the skill library + effectiveness), Settings,
...
release is `v0.8.1` — the Vitrine harvest: conversation-history replay
for chat sessions, skill trigger injection so skills are actually
used, a deterministic identifier backstop in the completion judge,
MCP tools in team-specialist hands, and a dozen honesty fixes from one
day of live operator walkthrough (see the CHANGELOG):
