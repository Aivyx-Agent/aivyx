...
//! `~/.aivyx/tool-processes/contacts/config.toml`.
//!
...
//! # (the full read-write `contacts` scope).
...
pub enum ConfigFileError {
    ...
    #[error("config file at {path:?} not found — create it with operator's OAuth client_id + client_secret + redirect_uri (see `aivyx-contacts help`)")]
...
/// Default path: `$HOME/.aivyx/tool-processes/contacts/config.toml`.
...
pub fn load_oauth_config(path: &Path) -> Result<OAuthConfig, ConfigFileError> {
    ...
    // on absence; populate contacts default here.
    if cfg.scopes.is_empty() {
        cfg.scopes = crate::DEFAULT_CONTACTS_SCOPES
            .iter()
            .map(|s| (*s).to_string())
            .collect();
    }
...
"#,
        ...
        assert_eq!(cfg.scopes.len(), crate::DEFAULT_CONTACTS_SCOPES.len());
