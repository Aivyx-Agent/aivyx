//! Operator-facing CLI for the Gmail OAuth flow.
//!
//! Phase 123 Task 3. Three subcommands:
//!
//! - `aivyx-gmail auth init` — runs the auth-code flow. Loads
...
//!   them via [`crate::oauth::storage::save_tokens`].
//!
//! - `aivyx-gmail auth status` — loads tokens (if any) and
//!   prints a human-readable health report: granted scopes,
//!   expiry, refresh-available?, time-since-init.
//!
//! - `aivyx-gmail auth revoke` — calls Google's revoke
...
//!   config from `~/.aivyx-pa/tool-processes/gmail/config.toml`.
