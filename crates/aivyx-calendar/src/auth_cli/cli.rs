//! CLI argument parsing for `aivyx-calendar auth ...`.
//!
...
pub enum AuthMode {
    /// `aivyx-calendar auth init` — run the OAuth flow.
    Init,
    /// `aivyx-calendar auth status` — print token health.
    Status,
    /// `aivyx-calendar auth revoke` — revoke + delete tokens.
    Revoke,
}
...
fn parse_auth(args: &[String]) -> Result<BinaryMode, String> {
    ...
    match sub {
        ...
        "" => Err(
            "`aivyx-calendar auth` requires a subcommand; \
...
pub fn help_text() -> &'static str {
    "aivyx-calendar — Calendar third-party tool process for Aivyx PA

USAGE:
    aivyx-calendar [SUBCOMMAND]
...
SUBCOMMANDS:
    ...
    auth revoke    Call Google's revoke endpoint and delete the
                   ...
                   before any Calendar tool will work.
