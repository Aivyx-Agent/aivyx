...
- [ ] `cargo clippy --workspace --all-targets -- -D warnings` — **known, not clean, and not this plan's fault**: Task 6's own investigation (`git stash` before/after comparison against the branch's base commit `a1019f67`) found `trigger.rs:223` (`clippy::result_unit_err`) and several unrelated crates (`aivyx-obsidian`, plus `aivyx-audit`/`aivyx-core` test-compile fallout from that same failure) already fail this exact command on `main`, untouched by any of this plan's 6 tasks — this worktree has no `.git/hooks/pre-commit` installed, so the documented `-D warnings` gate was never actually enforced at any of this lineage's commit times. The final reviewer should confirm `cargo clippy -p aivyx-channel -p aivyx-config -p aivyx-ipc --all-targets -- -D warnings` (the 3 crates this plan actually touches) is clean **except** for `trigger.rs:223` specifically, and should not chase the wider pre-existing workspace drift as part of this plan's own scope — log it to the ecosystem backlog instead (along with getting `scripts/install-hooks.sh` run in dev worktrees so this class of drift can't recur unnoticed). One pre-existing clippy issue Task 6 DID find genuinely in-scope (`team_mission_driver.rs:43`, an unused `TeamStepState` import at production scope, only used inside `#[cfg(test)]`) was fixed as part of Task 6's own commit.
...
- [ ] Confirm this plan's own Global Constraints held: `git diff --stat main..HEAD` should show changes confined to `crates/aivyx-config/src/lib.rs`, `crates/aivyx-config/src/tests.rs` (Task 1's own real deviation — the crate's tests live in a separate file, not inline in `lib.rs`; approved by Task 1's review), `crates/aivyx-channel/src/{schedule.rs,daemon_scheduler.rs,team_mission_driver.rs,schedule_tool.rs}`, `crates/aivyx-ipc/src/team_mission.rs`, `crates/aivyx-channel/src/daemon_server.rs` (Task 4's own real deviation — `run_scheduler`'s actual call site, not `aivyx-cli/src/bin/aivyx.rs` as originally guessed; approved by Task 4's review), and `crates/aivyx-cli/src/bin/aivyx.rs` (the daemon-startup wiring touched by Task 5) — no channel-adapter crate, no new capability base, no Studio (`aivyx-web`) changes. (This bullet's own file list was corrected during Task 6, once the real diff was visible — the original list, written before any task ran, didn't anticipate `tests.rs`/`daemon_server.rs`.)
