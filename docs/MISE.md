...
| **MI.1** | **`aivyx connect kitchen`** | The non-OAuth onboarding branch: render+write `[kitchen_db]` config `0600`, `append_tool_process("kitchen", "aivyx-kitchen-toolkit")` (idempotent), plant `KITCHEN_BOH_TOML` + set `[team] config_path` (no-clobber), probe KitchenDB via `KitchenClient` (mock-tested). Interactive prompts + `--base-url`/`--api-key`/`--organization-id` (or env) for headless installs. `aivyx-cli` gains the kitchen deps. |
