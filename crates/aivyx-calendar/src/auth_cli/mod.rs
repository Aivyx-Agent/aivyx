//! Operator-facing CLI for the Calendar OAuth flow.
//!
//! Phase 123 Task 3. Three subcommands:
//!
//! - `aivyx-calendar auth init` — runs the auth-code flow. Loads
...
//!
//! - `aivyx-calendar auth status` — loads tokens (if any) and
//!   prints a human-readable health report: granted scopes,
//!   expiry, refresh-available?, time-since-init.
//!
//! - `aivyx-calendar auth revoke` — calls Google's revoke
