...
- **Corrected during Task 3 — `aivyx-web` DOES compile natively in this environment.** The assumption below (this plan's original text) was wrong: `cargo check -p aivyx-web` and `cargo clippy -p aivyx-web --all-targets -- -D warnings` both work with no `wasm32-unknown-unknown` target installed — Dioxus's own macro-generated code type-checks against the host target fine; only the actual wasm binary/`just build-web` needs the real target. This was discovered when Task 3's implementer ran it anyway and it worked, surfacing 3 real compile errors in already-"Approved" Task 2 code that static review alone had missed (fixed in `be8aa36d`). **From here on, every web task (4, and re-verifying 6) must run `cargo check -p aivyx-web` and `cargo clippy -p aivyx-web --all-targets -- -D warnings` as real verification steps, not skip them.** **Corrected again during the final whole-branch review — `just build-web`/`just check-web` ALSO work here, via a second toolchain.** The system `/usr/bin/cargo` (Arch's own Rust, used for every other command in this plan) genuinely lacks the `wasm32-unknown-unknown` target and can't install it — but a separate, already-installed rustup toolchain at `~/.rustup/toolchains/stable-x86_64-unknown-linux-gnu` has the target's std library present. Prepend it to `PATH` (`export PATH="$HOME/.rustup/toolchains/stable-x86_64-unknown-linux-gnu/bin:$HOME/.cargo/bin:$PATH"`) to run the *real* `cargo check -p aivyx-web --target wasm32-unknown-unknown` and `dx bundle --release --platform web` (after `cargo install dioxus-cli --version 0.6.3 --locked` once, if `dx` isn't already on `PATH`) — both ran clean for real at the end of this branch, and the rebuilt bundle was committed to `crates/aivyx-web/dist/` (matching this repo's own existing convention of checking in the pre-built bundle, `a13c4576`). GPU/a live browser session are still genuinely unavailable in this sandbox — an operator's own visual/UX walkthrough remains the one thing that can't be automated here — but "we have zero access to the real wasm toolchain" was, a second time, simply false. Future sessions in this repo doing `aivyx-web` work should check for this rustup toolchain before assuming the wasm target is unreachable.
