...
use aivyx_obsidian::tools::{ObsidianGetNote, ObsidianSearch};
...
async fn run_ipc_loop() -> ExitCode {
    ...
    let tools: Vec<Arc<dyn Tool>> = vec![Arc::new(ObsidianSearch::new(Arc::clone(&client))), Arc::new(ObsidianGetNote::new(Arc::clone(&client)))];
