...
| **CD.1** | **stdio `env` + interpolation** | `RawMcpServer.env` → `McpServerConfig.env: Vec<(String,String)>` with `${VAR}` host-env resolution at load (unset var = config error); thread into `StdioTransport::start` via `cmd.envs(...)`. Unblocks the GitHub MCP server. |
| **CD.2** | **SSE/HTTP `headers` + interpolation** | `RawMcpServer.headers` → `McpServerConfig.headers`; thread into the SSE + Streamable-HTTP `reqwest` builders (operator headers can't clobber protocol headers). Unblocks remote authenticated servers. |
