//! `aivyx-drive auth init` — run the OAuth authorization-code
//! flow.
//!
...
//!    `~/.aivyx/tool-processes/drive/config.toml`.
...
pub async fn await_callback(
    ...
    let fut = async {
        ...
        write_browser_response(
            &mut write_half,
            "Aivyx Drive — authorization received",
...
pub async fn run_auth_init(
    ...
    println!(
        "\n\
         Aivyx Drive — OAuth consent required\n\
         ------------------------------------\n\
    ...
    println!(
        "Tokens saved to {token_path:?}.\n\
         Granted scope: {}\n\
         Run `aivyx-drive auth status` to inspect.",
...
mod tests {
    ...
    fn sample_config() -> OAuthConfig {
        ...
        // drive-specific default set so consent-URL
        ...
        .with_scopes(crate::DEFAULT_DRIVE_SCOPES.iter().copied())
    }
