...
**Side finding from the live data (not yet acted on):** the live catalog surfaced `team.run` (the Nonagon team-mission tool) at **Kernel** tier — `min_for_scope` is correct given the ceiling code (its base is absent from `CEILING_TRUSTED`/`CEILING_SEMITRUSTED`/`CEILING_UNTRUSTED`, present only via `CEILING_KERNEL`'s blanket `KNOWN_BASES` inclusion), but this looks like it could be an unintentional gap rather than deliberate: `team.delegate`/`team.message` sit right next to it in `CEILING_TRUSTED` (commented "Trusted-tier default"), and TOOLS.md's own table currently groups `team.run`/`team.delegate`/`team.message` together as one "Trusted" row. If `team.run` really is Kernel-only, no Trusted-tier chat channel can ever invoke it — worth the operator's eyes before anyone assumes it's reachable from normal chat. Not fixed here (out of this chapter's scope; the rig's own audit history shows `calls: 0` for it, so nothing is currently broken in practice).
