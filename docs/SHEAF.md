...
> **Status:** ✅ **COMPLETE (SH.0–SH.6).** The third **new-tools breadth** chapter,
...
| **SH.3** ✅ | **`data.pdf`** | DONE — **not deferred**: `pdf-extract` 0.7 + `lopdf` 0.34 (MIT) and their whole transitive tree cleared `cargo deny` (licenses + advisories) cleanly. Reuses the `ReaderSandbox` spine (same `fs.read` gate). `{path, text, pages, char_count, truncated}` — text extracted in-memory from the sandbox-read bytes (`pdf-extract`), page count best-effort via `lopdf` (0 if the structure is unreadable but text still extracted), `max_chars` cap (default 100k, hard 1M). **Text layer only, no OCR** — a scanned/image PDF yields empty text, reported not faked. `pdf-extract` returns `Err` (not a panic) on garbage, verified. **Registered in the daemon** beside the other readers (shared sandbox). `docs/TOOLS.md` entry added. 4 new tests (cap/ceiling/non-pdf-error/metadata); 24 crate tests; binary builds; clippy `-D warnings` clean. Real-PDF extraction is exercised in SH.5's live-verify. |
| **SH.4** ✅ | **Catalog + quality sweep** | DONE. `docs/TOOLS.md` carries a dedicated "Structured-data readers (infrastructure)" section with all three readers (scope `fs.read`), kept current across SH.1–SH.3. New `crates/aivyx-dataread/tests/tool_quality.rs` sweeps the three readers through Atlas's `check_tool_quality` (extending the AT.3 guard to this crate, as Abacus did for the toolkit tier) — zero metadata issues. Sheaf added **no new `KNOWN_BASES` base** (readers reuse `fs.read`), so the `tools_catalog_documents_every_known_base` drift-guard stays green untouched. clippy `-D warnings` clean. |
| **SH.5** ✅ | **Finalize** | DONE. **Live-verified** via `tests/live_verify.rs`: each reader's `execute()` is driven through a real `ToolContext` against a **real sample file** in a temp sandbox — `data.csv` (written inline) → correct headers/rows; `data.xlsx` (generated with the `rust_xlsxwriter` dev-dep) → correct sheet/headers/rows; `data.pdf` (generated with the already-present `lopdf`) → extracted `"Hello Sheaf PDF"`, `pages: 1`; plus a `../../../../etc/passwd` path **refused** through `execute()` (the inherited `fs.read` fence). A hand-crafted xref-less PDF was confirmed to *fail* cleanly first, motivating the `lopdf`-generated fixture. Full workspace suite + clippy `-D warnings` + `cargo deny` (advisories/bans/licenses/sources) green; `docs/ATLAS.md` §6 readers ✅; chapter memory recorded; status → COMPLETE. 29 crate tests total (24 unit + 4 live-verify + 1 quality sweep). |
| **SH.6** ✅ | **`data.xlsx.write` + `data.pdf.write`** | DONE (2026-07-07) — the write-side symmetric to SH.2/SH.3, from an operator-confirmed tools-system audit. Same governance move as the readers, mirrored: reuse the **existing `fs.write` capability**, not a new base — `ReaderSandbox` gained `scope_for_write`/`resolve_write_target`/`commit_write` (lexical + canonical fences matching `FsWriteTool`'s precedent, auto-create parent dirs, refuse to clobber an existing file unless `overwrite: true`, atomic temp-file-then-rename, Portcullis `classify_write` reused). `data.xlsx.write` — `rows: string[][]` (+ optional `headers`/`sheet_name`) via `rust_xlsxwriter` (promoted from a SH.5 dev-only test-fixture dependency to a real one). `data.pdf.write` — plain single-Helvetica-font text flow with automatic word-wrap + pagination onto US-Letter pages via `lopdf` (the same low-level content-stream API SH.5's PDF test fixture already used) — deliberately modest, matching the reader's "text layer only, no OCR" honesty: approximate (not glyph-exact) word-wrap, no images/tables/rich formatting. **Round-trip proven**: the xlsx writer's own test reads its output back through `parse_xlsx`; the PDF writer's through `pdf_extract::extract_text_from_mem`. 20 new tests (word-wrap/pagination/overwrite-refusal/metadata/round-trip); both tools registered in the daemon beside the readers (shared sandbox); `docs/TOOLS.md` updated; full workspace suite + clippy `-D warnings` green, no tool-count assertion broke. |
...
- **OQ-3 — PDF extractor + license (SH.3).** ✅ **Resolved: `pdf-extract` (text) +
  `lopdf` (page count), both MIT.** The full transitive tree cleared `cargo deny`,
  so `data.pdf` shipped (not deferred). Text layer only, no OCR.
