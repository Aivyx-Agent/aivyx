...
## Status (v0.7.18 — source-available, BUSL-1.1, 2026-06-30)
...
| Release pipeline | **Active** — on each version tag, cargo-dist builds the CLI (Linux x86_64/aarch64 musl + macOS x86_64/aarch64) and a separate workflow builds the **desktop app** (`.deb` + macOS `.app`); both attach to the GitHub Release. Latest is **`v0.7.18`** (the agent appends an honest note when it claims an action — saved to memory, scheduled, notified — it didn't actually take) via the [shell installer](docs/INSTALL.md#shell-installer-recommended) or the [desktop app](docs/INSTALL.md#desktop-app) |
...
release is `v0.7.18` (the agent appends an honest note when it claims an
action — saved to memory, scheduled, notified — it didn't actually take):
