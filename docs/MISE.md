...
> assembles it by hand — write `~/.aivyx-pa/tool-processes/kitchen/config.toml`,
> append a `[[tool_process]]`, point `[team] config_path` at the BOH pack, hope
> KitchenDB is reachable. Mise turns that into **`aivyx-pa connect kitchen`**: a
...
> success — plus **`aivyx-pa doctor`** checks so the operator can see the vertical is
...
for Google services — `aivyx-pa connect gmail` collapses "make an OAuth app →
...
### `aivyx-pa connect kitchen` — a non-OAuth branch of the existing `connect` command
...
1. **Write** `~/.aivyx-pa/tool-processes/kitchen/config.toml` (`0600`) with the
...
2. **Auto-wire** `[[tool_process]] name = "kitchen", command = "aivyx-kitchen-toolkit"`
   into `aivyx-pa.toml` (skipped if already present — `tool_process_present`).
3. **Plant the pack**: write the bundled `KITCHEN_BOH_TOML` (the
   `aivyx-kitchen` asset) to a `kitchen-boh.toml` beside `aivyx-pa.toml` and set
...
### `aivyx-pa doctor` learns the kitchen vertical
...
**In:** `aivyx-pa connect kitchen` (non-OAuth: config write + `[[tool_process]]`
...
`aivyx-pa doctor` kitchen section (MI.2); the `aivyx-cli` → `aivyx-kitchen` /
...
- **OQ-1 — pack-plant location (MI.1).** `kitchen-boh.toml` beside `aivyx-pa.toml`
  (locked lean — matches Roster's conventional discovery + `[team] config_path`)
  vs. inside `~/.aivyx-pa/`. Revisit if the operator runs from a non-cwd config.
...
#    …or headless:
AIVYX_PA_KITCHEN_BASE_URL=https://your-kitchen/rest/v1 \
...
`connect kitchen` writes `~/.aivyx-pa/tool-processes/kitchen/config.toml` (0600),
...
`aivyx-pa connect kitchen` — and the whole vertical is wired, planted, and proven
reachable, with `aivyx-pa doctor` to confirm it at a glance. The first vertical stops
