//! `aivyx-drive` binary entry point.
//!
//! Phase 129 Task 3 ships the operator-facing CLI dispatch
//! plus the IPC-loop scaffolding. Per-tool implementations
//! land in Tasks 4-10. Until then the IPC mode registers
//! zero tools and exits cleanly so the binary is
//! operator-installable for the auth flow before the full
//! tool surface is ready.

use std::process::ExitCode;
use std::sync::Arc;
use std::time::Duration;

use aivyx_core::Tool;
use aivyx_drive::auth_cli::{
    cli::{help_text, parse_cli_args_from, AuthMode, BinaryMode},
    config_file::{default_config_path, load_oauth_config},
    init::{generate_state_token, run_auth_init},
    revoke::{run_auth_revoke, GOOGLE_REVOKE_ENDPOINT},
    status::run_auth_status,
};
use aivyx_drive::tools::{
    DriveCreateFolder, DriveDeleteFile, DriveDownloadFile, DriveGetMetadata,
    DriveListDrives, DriveListFolder, DriveRecentChanges, DriveRecentFiles,
    DriveSearch, DriveUploadFile,
};
use aivyx_drive::{
    default_token_path, load_tokens, run_multi_tool_subprocess, DriveClient,
};

/// Operator-facing default for how long `auth init` waits
/// for the browser callback. Mirrors aivyx-gmail's 5-minute
/// budget.
const AUTH_INIT_CALLBACK_TIMEOUT: Duration = Duration::from_secs(300);

#[tokio::main]
async fn main() -> ExitCode {
    let argv: Vec<String> = std::env::args().collect();
    let mode = match parse_cli_args_from(&argv) {
        Ok(m) => m,
        Err(e) => {
            eprintln!("aivyx-drive: {e}");
            eprintln!("Run `aivyx-drive help` for usage.");
            return ExitCode::from(2);
        }
    };

    match mode {
        BinaryMode::Help => {
            println!("{}", help_text());
            ExitCode::SUCCESS
        }
        BinaryMode::Auth(AuthMode::Init) => run_init().await,
        BinaryMode::Auth(AuthMode::Status) => run_status().await,
        BinaryMode::Auth(AuthMode::Revoke) => run_revoke().await,
        BinaryMode::IpcLoop => run_ipc_loop().await,
    }
}

async fn run_init() -> ExitCode {
    let config_path = match default_config_path() {
        Ok(p) => p,
        Err(e) => {
            eprintln!("aivyx-drive auth init: {e}");
            return ExitCode::from(2);
        }
    };
    let config = match load_oauth_config(&config_path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("aivyx-drive auth init: {e}");
            return ExitCode::from(2);
        }
    };
    let token_path = match default_token_path() {
        Some(p) => p,
        None => {
            eprintln!("aivyx-drive auth init: $HOME unset; cannot resolve token path");
            return ExitCode::from(2);
        }
    };
    let state = generate_state_token();
    match run_auth_init(&config, &token_path, AUTH_INIT_CALLBACK_TIMEOUT, &state).await {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("aivyx-drive auth init: {e}");
            ExitCode::from(1)
        }
    }
}

async fn run_status() -> ExitCode {
    let token_path = match default_token_path() {
        Some(p) => p,
        None => {
            eprintln!("aivyx-drive auth status: $HOME unset; cannot resolve token path");
            return ExitCode::from(2);
        }
    };
    match run_auth_status(&token_path).await {
        Ok(report) => {
            print!("{report}");
            ExitCode::SUCCESS
        }
        Err(e) => {
            eprintln!("aivyx-drive auth status: {e}");
            ExitCode::from(1)
        }
    }
}

async fn run_ipc_loop() -> ExitCode {
    let config_path = match default_config_path() {
        Ok(p) => p,
        Err(e) => {
            eprintln!("aivyx-drive (ipc): {e}");
            return ExitCode::from(2);
        }
    };
    let oauth_config = match load_oauth_config(&config_path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("aivyx-drive (ipc): {e}");
            return ExitCode::from(2);
        }
    };
    let token_path = match default_token_path() {
        Some(p) => p,
        None => {
            eprintln!("aivyx-drive (ipc): $HOME unset; cannot resolve token path");
            return ExitCode::from(2);
        }
    };
    let tokens = match load_tokens(&token_path).await {
        Ok(Some(t)) => t,
        Ok(None) => {
            eprintln!(
                "aivyx-drive (ipc): no tokens at {token_path:?} — run `aivyx-drive auth init` first"
            );
            return ExitCode::from(2);
        }
        Err(e) => {
            eprintln!("aivyx-drive (ipc): token load failed: {e}");
            return ExitCode::from(2);
        }
    };

    let client = Arc::new(DriveClient::new(
        reqwest::Client::new(),
        oauth_config,
        tokens,
        token_path,
    ));

    // Phase 129 Q2b — seven-tool surface (search /
    // get_metadata / list_folder / create_folder /
    // download / upload / delete). Phase 145 adds the
    // LLM-ergonomic recent_* shapes mirroring
    // Phase 141's calendar.upcoming pattern. Phase 148
    // closes Phase 145's honest-debts: drive.list_drives
    // (shared/Team Drives enumeration mirroring
    // calendar.list_calendars).
    let tools: Vec<Arc<dyn Tool>> = vec![
        Arc::new(DriveSearch::new(Arc::clone(&client))),
        Arc::new(DriveGetMetadata::new(Arc::clone(&client))),
        Arc::new(DriveListFolder::new(Arc::clone(&client))),
        Arc::new(DriveCreateFolder::new(Arc::clone(&client))),
        Arc::new(DriveDownloadFile::new(Arc::clone(&client))),
        Arc::new(DriveUploadFile::new(Arc::clone(&client))),
        Arc::new(DriveDeleteFile::new(Arc::clone(&client))),
        Arc::new(DriveRecentFiles::new(Arc::clone(&client))),
        Arc::new(DriveRecentChanges::new(Arc::clone(&client))),
        Arc::new(DriveListDrives::new(Arc::clone(&client))),
    ];

    match run_multi_tool_subprocess(tools, "aivyx-drive").await {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("aivyx-drive (ipc): harness exited with error: {e}");
            ExitCode::from(1)
        }
    }
}

async fn run_revoke() -> ExitCode {
    let token_path = match default_token_path() {
        Some(p) => p,
        None => {
            eprintln!("aivyx-drive auth revoke: $HOME unset; cannot resolve token path");
            return ExitCode::from(2);
        }
    };
    let client = reqwest::Client::new();
    match run_auth_revoke(&client, GOOGLE_REVOKE_ENDPOINT, &token_path).await {
        Ok(outcome) => {
            println!("{}", outcome.message);
            ExitCode::SUCCESS
        }
        Err(e) => {
            eprintln!("aivyx-drive auth revoke: {e}");
            ExitCode::from(1)
        }
    }
}
