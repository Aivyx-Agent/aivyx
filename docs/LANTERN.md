...
> **Status:** 🟡 **PLANNED (LN.0–LN.4).** The web port of [Chapter Conduit](CONDUIT.md)'s
> `aivyx mcp status`: a **read-only Studio screen** that shows each configured MCP
...
Conduit made operator-added MCP servers work and gave the CLI an `aivyx mcp status`
...
**panel**. Governance (adding/removing servers) stays in `aivyx.toml`; the screen
...
`$XDG_DATA_HOME/aivyx/mcp-status.json`, with the struct + path resolver currently
...
The daemon's startup writer (CD.3) and the CLI's `aivyx mcp status` reader both call
...
(add/remove/enable a server — `aivyx.toml` stays the source; a future write-screen
...
| **LN.1** | **Shared types + snapshot home** | `McpServerStatusView` + `GetMcpStatus` query/response in `aivyx-ipc`; lift the snapshot path resolver + read/write helpers to a native module (`aivyx-channel`) over the ipc view type; refactor `aivyx-cli`'s CD.3 writer + `aivyx mcp status` reader onto them (output byte-identical). Round-trip + path tests. |
| **LN.2** | **Daemon handler** | `daemon_server.rs` answers `GetMcpStatus` by reading the snapshot → `Vec<McpServerStatusView>` + `captured_unix`; absent file → empty response (not an error). Handler test. |
| **LN.3** | **Studio screen** | `View::Mcp` + nav item + `McpPanel` + `McpState` + `mcp_query()` + the `GetMcpStatus` response arm; server cards (status pill, tool count, collapsible error/stderr); empty/loading states. |
| **LN.4** | **Finalize** | `dx build --release` (WASM) clean; serve + verify the screen renders real snapshot data in a browser; full workspace suite + clippy + `cargo deny` green; commit `dist/`; chapter memory; status → COMPLETE. |

**Discipline:** LN.1 lands the shared snapshot home + wire types so LN.2/LN.3 build on
one definition; the `aivyx-cli` refactor must keep `aivyx mcp status` byte-identical
