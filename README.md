...
## Status (v0.7.40 — source-available, BUSL-1.1, 2026-07-04)
...
| Release pipeline | **Active** — on each version tag, cargo-dist builds the CLI (Linux x86_64/aarch64 musl + macOS x86_64/aarch64) and a separate workflow builds the **desktop app** (`.deb` + macOS `.app`); both attach to the GitHub Release. Latest is **`v0.7.40`** (Chapter Strop — skill effectiveness now measures how well a skill serves, not how often it runs: hollow claimed-but-not-done skill turns and operator-corrected skill turns fold negative, so the governed refinement loop fires organically at default thresholds, live-verified on the test rig) via the [shell installer](docs/INSTALL.md#shell-installer-recommended), the [desktop app](docs/INSTALL.md#desktop-app), or the [WSL distro](docs/INSTALL.md#windows-wsl2-or-docker) |
...
release is `v0.7.40` (Chapter Strop — a graded skill-effectiveness
signal, so the governed skill-refinement loop fires organically at
default thresholds; live-verified on the test rig):
