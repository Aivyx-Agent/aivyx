//! `aivyx-calendar auth init` — run the OAuth authorization-code
//! flow.
//!
...
pub async fn await_callback(
    ...
    let fut = async {
        ...
        write_browser_response(
            &mut write_half,
            "Aivyx PA Calendar — authorization received",
...
pub async fn run_auth_init(
    ...
    println!(
        "\n\
         Aivyx PA Calendar — OAuth consent required\n\
         ------------------------------------\n\
    ...
    println!(
        "Tokens saved to {token_path:?}.\n\
         Granted scope: {}\n\
         Run `aivyx-calendar auth status` to inspect.",
